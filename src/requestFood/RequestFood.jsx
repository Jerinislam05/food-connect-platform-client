...
const RequestFood = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-4xl font-extrabold text-gradient bg-gradient-to-r from-teal-500 to-green-600 text-transparent bg-clip-text text-center mb-8 font-serif animate-bounce">
        <FaUtensils className="inline-block mr-2" /> ~Submitted Requests~
      </h2>
      {requests.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {requests.map((request, index) => (
            ...
            >
              {/* Badge */}
              <span className="absolute top-2 right-2 px-3 py-1 text-xs font-bold text-white bg-teal-500 rounded-full shadow-md">
                Request #{index + 1}
              </span>
